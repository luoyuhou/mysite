...
def connect(client):
    ...
    while True:
        ...
        for r in rs:
            if r is client:
                # 接受服务器发来的消息
                data = client.recv(SOCKET_SERVER['buffer_size'])
                if data.decode() == '\n':
                    # 如果消息为回车，聊天室关闭
                    client.close()
...
def login(client):
    ...
    # 再连接到服务器，传送用户名以检验
    print('addr', SOCKET_SERVER['addr'])
    # client.connect(SOCKET_SERVER['addr'])  # 连接到服务器地址
    client.connect(('127.0.0.1', 8090))  # 连接到服务器地址
    print(txt_connect_to, SOCKET_SERVER['addr'])
